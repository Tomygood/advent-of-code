...
func energized_tiles(start Beam, grid [][]rune) (int, [][]rune) {
	...
	for len(beams) != 0 {
		beams = step(beams, grid, &already_found)
		for _, b := range beams {
			energized[b.y][b.x] = '#'
		}
	}
...
func main() {
	...
	for x := range len(grid) {
		n, g := energized_tiles(Beam{-1, x, 1, 0}, mat)
		if n > max {
			max = n
			max_grid = g
		}
		n, g = energized_tiles(Beam{len(grid), x, -1, 0}, mat)
		if n > max {
			max = n
			max_grid = g
		}
		n, g = energized_tiles(Beam{x, 0, 0, 1}, mat)
		if n > max {
			max = n
			max_grid = g
		}
		n, g = energized_tiles(Beam{x, len(grid), 0, -1}, mat)
		if n > max {
			max = n
			max_grid = g
		}
